...
Page({
  ...
  data: {
    ...
    answerUrl: 'index.php?r=question/exam_submit' ,//交卷URL
    answers:{
      onLoadUrl: 'index.php?r=question/get_id_items',//题目号链接
      start:0,//初始题号
      end:0,//结束题号
      allLists:[],//题号数据
      activeNum:0,//当前条数
      ...
      url: 'index.php?r=question/get_info',//题目详情链接
      isShowTip:false//默认是否显示alert
    }
  },
  //单选逻辑
  tapRadio:function(e){
  ...
  tapSelect:function(e){
    ...
    if(this.data.maxError+1 == this.data.answers.error){
      wx.showModal({
        title:'tip',
        content: `You have wrong ${this.data.answers.error} questions，fail to pass exam，do you continue？` ,
        showCancel:true,
        cancelText: 'continue',
        confirmText: 'submit',
  ...
  submitTip:function(){
    ...
    if(this.data.answers.allLists.length > this.data.answers.error + this.data.answers.success){
      wx.showModal({
        title:'alert',
        content: `you have answer ${this.data.answers.error + this.data.answers.success} questions，you have not finished ${this.data.answers.allLists.length - this.data.answers.error - this.data.answers.success} questions，do you sure submit？` ,
        showCancel:true,
        cancelText:'No',
        cancelColor:'#00bcd5',
        confirmText:'Yes',
      ...
      wx.showModal({
        title:'tip',
        content: 'This is the last question，after submit,you can see your mark',
        showCancel:false,
        confirmText:'I know',
        confirmColor:'#00bcd5',
        success: function(res) {
  ...
  setSubmit:function(){
    ...
    .then((data) =>{
      if(data.data.status == 1){
        wx.redirectTo({
          url: `/pages/answer/answer_mark/mark?subject=${this.data.subject}&type=${this.data.type}&time=${this.params.seconds}&mark=${data.data.data.score}&mid=${data.data.data.mid}&error=${this.data.answers.error}`
        })
      } else {
        wx.showModal({
          title: '错误',
